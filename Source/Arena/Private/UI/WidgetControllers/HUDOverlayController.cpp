...
{
	if (ARPGPlayerController* PC = Cast<ARPGPlayerController>(OwningActor))
	{
		PC->SkillActivatedDelegate.BindLambda(
			[this](const FGameplayTag& SkillTag)
			{
				if (SkillTag.MatchesTag(RPGGameplayTags::Input::Skill))
	...
	if (EnsureOwningEquipment())
	{
		OwningEquipmentComp->EquipmentList.EquipmentEntryDelegate.AddLambda(
			...
			{
				if (EquipmentEntry.SlotTag.MatchesTagExact(RPGGameplayTags::Equip::WeaponSlot))
				{
