...
class MAKHIA_API UMovementState : public UObject
{
	GENERATED_BODY()
...
public:
	...
	virtual void Initialize(UMovementStateMachine* InStateMachine, AMKHPlayerCharacter* InPlayerCharacter);
	...
	/** Returns the gameplay tag associated with a given movement state */
	static FGameplayTag GetTagForState(EMovementStateValue StateType);
...
protected:
	...
	UPROPERTY(BlueprintReadOnly, Category = "Movement State")
	TObjectPtr<AMKHPlayerCharacter> MKHPlayerCharacter;

	/** Helper function to get valid player character */
	UFUNCTION(BlueprintPure, Category = "Movement State")
	AMKHPlayerCharacter* GetPlayerCharacter() const { return MKHPlayerCharacter.Get(); }
