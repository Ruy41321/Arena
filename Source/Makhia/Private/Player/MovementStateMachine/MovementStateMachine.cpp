...
void UMovementStateMachine::BeginPlay()
{
	...
	{
		UE_LOG(LogTemp, Error, TEXT("MovementStateMachine: Owner is not a MKHPlayerCharacter!"));
		return;
	}
...
AMKHPlayerCharacter* UMovementStateMachine::GetValidPlayerCharacter() const
{
...
void UMovementStateMachine::PerformStateTransition(EMovementStateValue NewState)
{
	...
	// stampa server: jumpcomponent->bIsLanding o client: MKHPlayerCharacter->bIsLanding
...
void UMovementStateMachine::SyncCurrentStateTagToASC()
{
	AMKHPlayerCharacter* Player = GetValidPlayerCharacter();
	if (!Player)
	{
		return;
	}
	...
	{
		const FGameplayTag StateTag = UMovementState::GetTagForState(CurrentState);
